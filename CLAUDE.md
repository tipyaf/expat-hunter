...
5. `[ ]` Functional / e2e tests written and passing (see e2e rules below)
...
10. `[ ]` E2e specs updated for any modified frontend behaviour (see e2e rules below)
11. `[ ]` PR created on GitHub and linked to Shortcut story
12. `[ ]` feature-tracker.yaml updated → validated

### E2e test rules (mandatory for any story with frontend impact)
...
| **Coverage minimum** | Every page/flow listed in `specs/expat-hunter-ux.md` must have at least one e2e spec. New pages added without a spec require a follow-up story created immediately. |
